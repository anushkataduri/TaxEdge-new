...
export const authService = {
  findUserByMobile: (m: string) => authStorage.getUserByMobile(m),
  isUserRegistered: (m: string) =>
    Boolean(authStorage.getUserByMobile(m)?.passcode),
  ...
  async verifyOtp(
    ...
    } else if (existing && existing.passcode) {
      isExisting = true;
    } else {
  ...
  async checkUser(
    mobileNumber: string,
  ): Promise<{ exists: boolean; user?: DevUser }> {
    const clean = mobileNumber.replace(/\D/g, "");
    const existing = authStorage.getUserByMobile(clean);
    ...
    const checkRes = await authApi.checkUser(clean);
    return { exists: Boolean(checkRes?.exists), user: existing || undefined };
  },
  ...
  async loginWithPasscode(m: string, p: string): Promise<AuthResult> {
    ...
    if (!apiRes.success) {
      return {
        success: false,
        error: apiRes.message || "Invalid mobile number or passcode",
      };
    }
  ...
  async updatePassword(
    ...
    if (pass.length !== 6)
      return { success: false, error: "Passcode must be 6 numeric digits" };

    const apiRes = await authApi.updatePassword(clean, pass);
    if (!apiRes.success) {
      return {
        success: false,
        error: apiRes.message || "Failed to update password",
      };
    }
  ...
  async resetPasscode(
    ...
    if (!apiRes.success) {
      return {
        success: false,
        error: apiRes.message || "Failed to reset passcode",
      };
    }
  ...
  isAuthenticated: () =>
    Boolean(
      authStorage.getSession().isLoggedIn &&
      authStorage.getSession().activeMobile,
    ),
  getActiveMobile: () => authStorage.getSession().activeMobile,
  getCurrentUser: (): DevUser | null => {
    const session = authStorage.getSession();
    return session.activeMobile
      ? authStorage.getUserByMobile(session.activeMobile)
      : null;
  },
  setAvatar: (uri: string | null) => {
    const u = authService.getCurrentUser();
